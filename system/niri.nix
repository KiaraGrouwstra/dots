...
let
  user = "kiara";
  inputs = {};
  kdl = import <niri-flake/kdl.nix> { inherit lib; };
  binds = import <niri-flake/parse-binds.nix> { inherit lib; };
  docs = import <niri-flake/generate-docs.nix> { inherit lib inputs; };
  settings = import <niri-flake/settings.nix> { inherit lib kdl binds docs inputs; };
    ...
    config = {
      lib.niri = {
        actions = lib.mergeAttrsList (
          ...
          }) (import pins.niri-flake.memo-binds)
        );
      };
      xdg.portal = {
        enable = true;
      ...
      xdg.configFile.niri-config = {
        ...
        source =
          pkgs.runCommand "config.kdl"
            {
              config = kdl.serialize.nodes config.programs.niri.config;
              passAsFile = [ "config" ];
          ...
          in
          {
            ...
            binds =
              ...
              lib.mapAttrs (_: str: { action.spawn = sh str; }) {
                ...
                # "Mod+Ctrl+D" = "${pkgs.anyrun}/bin/anyrun";
                "Mod+J" = lib.getExe pkgs.cosmic-launcher;
                "Mod+Shift+J" = lib.getExe pkgs.cosmic-app-library;
                "Mod+L" = lib.getExe pkgs.cosmic-greeter;
              }
              //
                lib.mapAttrs
                  (_: str: {
              ...
              // (binds {
                ...
                suffixes = {
                  ...
                  "D" = "column-left-or-last";
                  "F" = "column-right-or-first";
                };
              })
              // (binds {
                inherit substitutions;
